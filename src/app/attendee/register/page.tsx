...
export default function RegisterAttendance() {
  ...
  useEffect(() => {
    ...
    // Fetch the server date once just so we know which dates count as "Retroactive"
    async function fetchServerDate() {
      try {
        ...
        setTodayString(`${getPart('year')}-${getPart('month')}-${getPart('day')}`)
      } catch (e) {
        // Fallback to local date if server fetch fails
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] flex flex-col items-center justify-center p-4 md:p-8">
      <div className="w-full max-w-md bg-white rounded-xl shadow-md border-2 border-brand-burgundy overflow-hidden relative">
        
        <div className="bg-yellow-400 text-yellow-900 text-xs font-bold uppercase text-center py-1 tracking-wider">
          Testing Mode (All Time Restrictions Removed)
        </div>
