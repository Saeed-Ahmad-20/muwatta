...
export default function RegisterAttendance() {
  ...
  useEffect(() => {
    ...
    async function syncClock(attempt = 1) {
      ...
      try {
        ...
        const serverIso = await Promise.race([
          ...
          new Promise<never>((_, reject) =>
            setTimeout(() => reject(new Error('Clock sync timeout')), TIMEOUT_MS)
          ),
        ])
  ...
  useEffect(() => {
    ...
    const checkTimeAndDates = () => {
      ...
      const activeDateId =
        selectedDate || (unlockedDates.length > 0 ? unlockedDates[unlockedDates.length - 1].id : '')
      ...
      <div className="min-h-[calc(100vh-4rem)] flex flex-col items-center justify-center p-8 text-center">
        ...
        <p className="text-gray-500 font-medium">
          {syncStatus === 'retrying'
            ...
            : 'Synchronizing secure clock...'}
        </p>
        {syncStatus === 'retrying' && (
          <p className="text-xs text-gray-400 mt-2">
            If this persists, please check your internet connection and refresh the page.
          </p>
        )}
      </div>
    )
  ...
  if (appState === 'concluded') {
    return (
      <div className="min-h-[calc(100vh-4rem)] flex flex-col items-center justify-center p-8 text-center bg-brand-burgundy text-brand-gold">
        <svg
          className="w-20 h-20 mb-6 text-brand-gold mx-auto"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
    ...
    return (
      <div className="min-h-[calc(100vh-4rem)] flex flex-col items-center justify-center p-8 text-center">
        ...
        <p className="text-gray-600 max-w-md mx-auto">
          Attendance tracking for the Muwatta Recital will open on Saturday, April 4th, 2026.
          ...
          {successData ? (
            <div className="text-center animate-in fade-in zoom-in">
              ...
              <p className="text-gray-600 mb-2">
                Jazakallah Khair,{' '}
                <strong className="text-brand-burgundy">{successData.attendee_name}</strong>!
              </p>
              ...
              {successData.isRetroactive && (
                <div className="text-xs bg-yellow-50 text-yellow-800 p-3 rounded-lg mt-3 mb-2 border border-yellow-200">
                  Because this is a past date, your attendance has been sent to an admin for manual
                  approval.
                </div>
              )}
              ...
              <div className="bg-gray-50 rounded-lg p-4 mb-6 mt-4 border border-gray-200">
                ...
                <p className="text-sm text-gray-700 mt-2 font-medium">
                  ...
                  {successData.newly_registered.am && (
                    <span className="font-bold text-brand-gold ml-1">AM</span>
                  )}
                  {successData.newly_registered.am && successData.newly_registered.pm && ' & '}
                  {successData.newly_registered.pm && (
                    <span className="font-bold text-brand-gold ml-1">PM</span>
                  )}
                </p>
                ...
                {(successData.already_registered.am || successData.already_registered.pm) && (
                  <div className="mt-3 p-2 bg-blue-50 border border-blue-100 rounded text-xs text-blue-800 font-medium">
                    ...
                    {successData.already_registered.am &&
                      successData.already_registered.pm &&
                      ' and'}
                  ...
                  {isEditingMessage ? (
                    ...
                    >
                      ...
                      <div className="absolute top-2 right-2 opacity-40 group-hover:opacity-100 transition-opacity">
                        <svg
                          className="w-4 h-4 text-brand-burgundy"
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                        >
            ...
            <form onSubmit={handleRegister} className="space-y-6">
              ...
              <div>
                ...
                {!isAmEnabled && !isPmEnabled && (
                  <p className="text-xs text-red-500 mt-2 text-center">
                    There are no active sessions available for this date yet.
                  </p>
                )}
              </div>
              ...
              <button
                ...
                disabled={
                  loading ||
                  (!isAmEnabled && !isPmEnabled) ||
                  (!selectedSessions.am && !selectedSessions.pm)
                }
